...
export async function createOrUpdateContact(contactData: HubSpotContact): Promise<CreateContactResponse> {
    try {
...
export async function trackFormSubmission(contactData: HubSpotContact): Promise<boolean> {
    try {
        if (!process.env.HUBSPOT_ACCESS_TOKEN) {
            console.warn('HubSpot access token not configured, skipping event tracking');
            return false;
        }
