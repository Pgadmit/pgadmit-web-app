...
import { Heart, MapPin, Users, Calendar, ExternalLink, Bookmark } from "lucide-react"
...
export function UniversityGrid() {
  ...
  const toggleBookmark = (universityId: string) => {
    setUniversities((prev) =>
      prev.map((uni) => (uni.id === universityId ? { ...uni, isBookmarked: !uni.isBookmarked } : uni)),
    )

    const university = universities.find((u) => u.id === universityId)
    toast({
      title: university?.isBookmarked ? "Removed from bookmarks" : "Added to bookmarks",
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {universities.map((university) => (
        <Card key={university.id} className="bg-card shadow-sm hover:shadow-md transition-shadow">
          <CardHeader className="pb-3">
            <div className="flex items-start justify-between">
              ...
              <Button
                ...
                className={university.isBookmarked ? "text-red-500" : "text-muted-foreground"}
              >
          ...
          <CardContent className="space-y-4">
            ...
            <Button className="w-full" onClick={() => viewDetails(university.id)}>
              View Details
