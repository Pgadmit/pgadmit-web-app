...
import { getUserStages, getCurrentStage, getNextSteps } from "@/lib/dashboard-utils"
      ...
      <CardContent>
        <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              {urgentSteps.map((step, index) => (
                <div key={index} className="flex items-center justify-between p-3 bg-background rounded-lg border">
                  <span className="text-sm font-medium text-foreground">{step.task}</span>
                  <Button size="sm" variant="outline" onClick={() => handleTaskComplete(step.task)}>
                    Start (+10 XP)
          ...
          {nextSteps.filter((step) => !step.urgent).length > 0 && (
            <div className="space-y-2">
              <div className="text-sm font-medium text-muted-foreground">Other Tasks</div>
              {nextSteps
                .filter((step) => !step.urgent)
                .map((step, index) => (
                  <div key={index} className="flex items-center justify-between p-3 bg-background/50 rounded-lg">
                    <span className="text-sm text-foreground">{step.task}</span>
                    <Button size="sm" variant="ghost" onClick={() => handleTaskComplete(step.task)}>
                      View (+5 XP)
                    </Button>
                  </div>
                ))}
